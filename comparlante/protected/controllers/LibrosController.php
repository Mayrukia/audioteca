...
class LibrosController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			...
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('create','update','admin','delete','partes','index','view','gridLibroGenero','AutoCompleteLookup','Vview','libroAutor','listarNarradores','autores','updateUrl','updateAutores','bibliotecaIdiomas','upload'),
				'users'=>array('@'),
				),
			
			/* Permite el acceso a solo usuarios "no logeados" */
			array('allow',
				'actions'=>array('index','Vview','libroAutor','bibliotecaIdiomas','view'),
				'users'=>array('?'),
				),
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Libros']))
		{
			...
			if($model->save())
			{
				...
				// Redirecciona si tiene partes
				if(isset($_POST["checkbox_1"]))
				{
					$this->redirect(array('partesLibros/create','id'=>$model->ID_LIBRO));
					//$this->redirect(array('partes','id'=>$model->ID_LIBRO));
				}
				else //redirecciona si no tiene partes
	...
	//Funcion que permite agregar partes a los libros
	
	public function actionPartes($id){

		$model 	= $this->loadModel($id);

		$this->render('partes',array('model'=>$model,
			));
	}
	...
	protected function gridLibroGenero($data,$row)
	{
		...
		public function actionBibliotecaIdiomas()
		{
			...
			$criteria = new CDbCriteria;
			$criteria->condition = "ID_IDIOMA = :id";
